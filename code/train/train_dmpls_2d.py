"""UNetCCT2D + DMPLS (Luo et al., MICCAI 2022) on ACDC/MSCMR's 2D
slice-supervised protocol.

DMPLS is the single-stage predecessor of DMSPS (``train_dmsps_2d.py``): the
same dual-branch pCE + dynamically mixed soft pseudo-label consistency loss
(``utils/dmpls.py``, re-exporting the verified ``utils/dmsps.py``
primitives), but trained end-to-end in one stage -- no stage-1/stage-2 split,
no uncertainty-guided label expansion. ``UNetCCT2D`` (``networks/unet_2d.py``)
is the same shared-encoder / main-decoder / dropout-perturbed-auxiliary-
decoder DB-Net architecture DMSPS uses.

Only sparse scribble labels contribute to optimization. Dense training
labels are accessed exclusively for model selection on a patient-level
holdout, exactly as in ``train_pce_2d.py``. Only ACDC/MSCMR (2D) are
implemented for DMPLS in this benchmark; there is no ``train_dmpls_3d.py``.
"""
...
from utils.dmpls import dmpls_step  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D dual-decoder U-Net from ScribbleBench scribbles with DMPLS"
    )
    ...
    parser.add_argument(
        "--lambda_pls", type=float, default=0.5,
        help="pseudo-label supervision weight (paper Eq. 4 lambda; default matches Sec. 3.1)",
    )
...
def validate_args(args):
    ...
    if args.lambda_pls < 0:
        raise ValueError("lambda_pls must be non-negative")
    return args
...
def train(args):
    ...
    output_dir = Path(
        args.output_dir or REPO_ROOT / "checkpoints" / "ScribbleBench_DMPLS" / args.dataset
    ).resolve()
    ...
    try:
        while step < args.max_iterations:
            ...
            for batch in loader:
                ...
                with amp_context:
                    loss, components = dmpls_step(model, image, target, ignore_index, args)
                ...
                if step % 20 == 0:
                    logging.info(
                        "iteration=%d/%d loss=%.6f pce=%.6f pls=%.6f alpha=%.3f lr=%.6g",
                        step, args.max_iterations, loss.item(), components["pce"], components["pls"],
                        components["alpha"], lr,
                    )
