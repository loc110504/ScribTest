...
# ACDC, ScribbleBench scribbles (dataset/ScribbleBench/ACDC), one summary
# CSV row per method:
...
# WORD (ACDC/MSCMR-only paper). See run_icassp2027_baselines_mscmr.sh for
# the MSCMR twin -- run both to reproduce the whole of Table 1.
#
...
# ACDC row), then evaluates only the ACDC half; the MSCMR script does the
# mirror image and, if you already ran this script first, reuses the same
...
# results/icassp2027_baselines_summary.csv, shared with the MSCMR script so
...
#   bash code/train/run_icassp2027_baselines_acdc.sh
...
echo "Run run_icassp2027_baselines_mscmr.sh next to complete Table 1."
