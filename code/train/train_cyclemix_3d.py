"""UNet3D + CycleMix (Zhang & Zhuang, CVPR 2022) on ScribbleBench.
...
this repository's ``UNet3D`` and ``ScribbleBench3DDataset``. See
...
paper lists as an admissible mix operator).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 3D U-Net from ScribbleBench scribbles with CycleMix"
    )
...
def train(args):
    ...
    model = UNet3D(in_chns=1, class_num=num_classes, feature_chns=args.feature_channels).to(device)
