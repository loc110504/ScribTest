"""Table 2 "In-sample, class-only" design-choice ablation
(``paper_icassp2027/main.tex``, "Ablating Trust Calibration"), on ACDC/
MSCMR's 2D slice-supervised protocol.

Standalone copy of ``train_voxtrust3d_2d.py`` -- deliberately **not** built
by modifying that file, or ``train_voxtrust3d_3d.py`` (whose ``voxtrust_step``
continues to reproduce only the base method and the four original Table 2
ladder arms unchanged) -- so this ablation lives entirely in new code,
exactly like the three ``train_voxtrust3d_2d_ablation_{raw_ratio,
extrapolate,top1conf}.py`` design-choice knockouts it sits alongside.

The only behavioral difference from the "Held-out, class-only" ladder arm
(``--ablation class_only``) is applied via
``utils.voxtrust3d_ablation_step.voxtrust_step_in_sample_class_only``: the
calibration buffer's ``(R_i, correct)`` records are read from ``Omega_sup``
(the pixels the student is directly trained on) instead of the withheld
``Omega_cal``, isolating whether calibrating against evidence the student
never fits differs from calibrating against evidence it does. Distance
conditioning stays off (class-only pooling), the Wilson bound and abstention
policy stay at their proposed-method defaults, and Trust-Advantage EMA (this
project's own extension, not part of what Table 2 ablates) is not exposed
here -- the EMA teacher update always uses the fixed ``--ema_decay`` rate.
See that function's own docstring for the two accepted simplifications this
evidence swap implies (singleton calibration "blocks", near-zero transfer
distance).

Reuses this benchmark's shared, unmodified infrastructure: ``VoxTrustSlice2DDataset``/
``create_model`` from ``train_voxtrust3d_2d.py`` (dataset/model construction,
not method logic) and ``build_val_dataset``/``resolve_case_split`` from
``train_pce_2d.py``. Only sparse labels in ``labelsTr`` (split further into
Omega_sup/Omega_cal) contribute to optimization; dense training labels are
...
from train.train_voxtrust3d_2d import VoxTrustSlice2DDataset, create_model  # noqa: E402
from utils.ema_optim import WeightEMA  # noqa: E402
from utils.ramps import sigmoid_rampup  # noqa: E402
from utils.voxtrust3d import RollingCalibrationBuffer, fit_distance_bins, stable_seed, strong_intensity_augment_2d  # noqa: E402
from utils.voxtrust3d_ablation_step import voxtrust_step_in_sample_class_only  # noqa: E402
...
ABLATION_NAME = "in_sample_class_only"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train VoxTrust-3D/DCC on ScribbleBench ACDC/MSCMR, class-only calibration read from "
            "Omega_sup (in-sample) instead of Omega_cal (held-out)"
        )
    )
    ...
    parser.add_argument(
        "--holdout_fraction", type=float, default=0.15,
        help="eta; still withheld from Omega_sup/pseudo-label transfer, just never read for calibration here",
    )
    ...
    parser.add_argument("--calibration_block_cap", type=int, default=64, help="m_max (unused; see docstring)")
...
def checkpoint_payload(model, model_ema, optimizer, scaler, calibrator, args, split, step, best_score):
    return {
        "schema_version": 1,
        "model_name": "unet_2d",
        "training_method": "voxtrust3d_ablation_{}".format(ABLATION_NAME),
        "model_config": {
            "in_chns": 1,
...
def train(args):
    ...
    output_dir = Path(
        args.output_dir
        or REPO_ROOT / "checkpoints" / "ScribbleBench_VoxTrust3D_dcc_ablation" / args.dataset / ABLATION_NAME
    ).resolve()
    ...
    if args.resume:
        step, best_score = restore_checkpoint(args.resume, model, model_ema, optimizer, scaler, calibrator, args, split)
    ...
    logging.info(
        "ablation=%s dataset=%s classes=%d ignore=%d train_slices=%d val_cases=%d patch=%s device=%s "
        "warmup_iters=%d rampup_iters=%d",
        ABLATION_NAME, args.dataset, num_classes, ignore_index, len(train_slice_positions), len(val_case_indices),
        args.patch_size, device, warmup_iters, rampup_iters,
    )
            ...
            for batch in loader:
                ...
                with amp_context:
                    loss_scrib, loss_pl, diagnostics = voxtrust_step_in_sample_class_only(
                        model, model_ema, batch, device, ignore_index,
                ...
                if should_checkpoint:
                    ...
                    if score > best_score:
                        best_score = score
                        payload = checkpoint_payload(model, model_ema, optimizer, scaler, calibrator, args, split, step, best_score)
                    ...
                    model_ema.train()
                    atomic_torch_save(
                        checkpoint_payload(model, model_ema, optimizer, scaler, calibrator, args, split, step, best_score),
                        output_dir / "last.pth",
                    )
                if step >= args.max_iterations:
                    break
