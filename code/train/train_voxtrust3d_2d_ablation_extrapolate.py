"""Table 2 "w/o abstention" design-choice ablation
...
ladder arms unchanged) -- so this ablation lives entirely in new code. The
only behavioral difference from the full method is one swap, applied via
``utils.voxtrust3d_ablation_step.voxtrust_step_knockout``: a candidate whose
per-(class, distance-stratum) cell has no cutoff meeting the target
precision, or whose distance exceeds that class's ``d_max``, borrows the
nearest non-abstaining bin's threshold (``utils.voxtrust3d.extrapolate_thresholds``)
instead of being rejected outright ("rejected rather than extrapolated",
Sec. 2.2 -- this ablation is the extrapolating alternative,
``abstain_policy="extrapolate"``). Held-out calibration and distance
conditioning are otherwise unchanged, and Trust-Advantage EMA (this
...
from utils.voxtrust3d_ablation_step import voxtrust_step_knockout  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train VoxTrust-3D/DCC on ScribbleBench ACDC/MSCMR, abstention replaced by extrapolation"
    )
    ...
    parser.add_argument(
        "--holdout_fraction", type=float, default=0.15, help="eta; held-out calibration fraction"
    )
            ...
            for batch in loader:
                ...
                with amp_context:
                    loss_scrib, loss_pl, diagnostics = voxtrust_step_knockout(
                        ...
                        calibration_estimator=CALIBRATION_ESTIMATOR,
