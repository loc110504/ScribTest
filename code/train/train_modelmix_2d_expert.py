...
MSCMR from the expert-scribble archive (see ``dataloader/expert_scribble_2d.py``),
*in addition to* (not instead of) ``train_modelmix_2d.py``'s ScribbleBench
pipeline.

Structurally this is ``train_modelmix_2d.py`` with every
``dataloader.scribblebench_*``/``train_pce_2d`` reference swapped for
``dataloader.expert_scribble_2d``/``train_pce_2d_expert`` -- see
``train_pce_2d_expert.py``'s module docstring for the full rationale. The
``modelmix_task_step`` mechanism itself (image-level mixup, model-level
mixup, vicinal regularization) is untouched.
...
``code/test/test_pce_2d_expert.py``.
"""
...
from train.train_modelmix_2d import modelmix_task_step  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Jointly train two UNet2D models for ACDC+MSCMR from the expert-scribble archive with ModelMix"
    )
...
def checkpoint_payload(model, args, dataset_name, split, step, best_score):
    """Model weights + bookkeeping only -- see ``train_modelmix_2d.py``'s
    function of the same name for why the optimizer/scaler state is
...
class TaskState:
    """Everything one task (ACDC or MSCMR) needs, mirroring
    ``train_modelmix_2d.py``'s class of the same name but sourced from the
    expert-scribble archive."""

    def __init__(self, dataset_name, args, device):
        self.dataset_name = dataset_name
        train_transform = RandomGenerator2D(args.patch_size)
        self.train_dataset = ExpertScribble2DDataset(
        ...
        train_case_indices, train_groups, val_groups, protocol = resolve_case_split(
            self.train_dataset.cases, dataset_name
        )
        self.val_case_indices = resolve_val_indices(self.val_dataset.cases, dataset_name)
        self.split = {
            ...
            "val_cases": [self.val_dataset.cases[index] for index in self.val_case_indices],
        }
        ...
        self.output_dir = Path(
            args.output_dir or REPO_ROOT / "checkpoints" / "ExpertScribble_ModelMix"
...
def train(args):
    ...
    logging.info(
        "ACDC(expert): train_slices=%d val_cases=%d | MSCMR(expert): train_slices=%d val_cases=%d | patch=%s device=%s",
