#!/usr/bin/env bash
# Train + test the fully-supervised (dense-mask) UNet2D upper-bound baseline
# on ACDC and MSCMR, appending each result to one summary CSV. Mirrors
# run_baselines.sh's structure and env-var conventions; ACDC/MSCMR-only (2D)
# since that is what train_fullsup_2d.py implements (see its module
# docstring) -- there is no WORD/3D counterpart in this benchmark.
#
# Unlike every scribble-supervised method in this repo, train_fullsup_2d.py
# optimizes directly against labelsTr_dense (not just labelsTr scribbles);
# this script measures the ceiling those methods are compared against, not
# another scribble-supervised method. The checkpoint is a plain UNet2D, so
# it is evaluated with the shared test_pce_2d.py, exactly like pCE/CycleMix/
# EFFDNet/SDT-Net/ModelMix/Bayes-WSS.
#
...
#   bash code/train/run_fullsup.sh
...
for dataset in "${datasets[@]}"; do
  if [ "$dataset" = "WORD" ]; then
    echo "Skipping WORD: train_fullsup_2d.py is ACDC/MSCMR-only (2D) in this benchmark" >&2
    continue
  fi

  ckpt_dir="$checkpoint_root/ScribbleBench_FullSup/$dataset"
  results_dir="$results_root/ScribbleBench_FullSup/$dataset"
  echo "=== [FullSup] dataset=$dataset: train ==="
  python "$script_dir/train_fullsup_2d.py" --dataset "$dataset" \
    --output_dir "$ckpt_dir" --batch_size "$batch_size" $amp_flag $device_flag $root_path_flag $extra_train_args
  echo "=== [FullSup] dataset=$dataset: test ==="
  ...
  append_row FullSup "$dataset" "" "$ckpt_dir/best.pth" "$results_dir/metrics.json"
done
