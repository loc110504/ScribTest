"""UNet2D + EFFDNet (Liu et al., MICCAI 2025) on ACDC/MSCMR's 2D
slice-supervised protocol.

Reuses ``effdnet_step`` from ``train_effdnet_3d.py`` unchanged: partial
cross-entropy, the dense teacher-pseudo-label cross-entropy, FBSL (grid-
based contrastive separation), and FADC (bounding-box copy-paste
augmentation) are all shape-agnostic over the spatial rank (see
``code/utils/effdnet.py``), so the exact same Mean-Teacher framework applies
to 2D slices with no changes.
...
is the *student*, so ``code/test/test_pce_2d.py`` evaluates ``best.pth``
directly. WORD stays a full-3D VNet pipeline, see ``train_effdnet_3d.py``.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D U-Net from ScribbleBench scribbles with EFFDNet"
    )
