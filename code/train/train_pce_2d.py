"""UNet2D + partial cross-entropy baseline with published legacy splits, for
ACDC/MSCMR's 2D slice-supervised protocol.

ACDC and MSCMR train as independent 2D slices (anisotropic spacing), the
standard protocol across the scribble-supervision literature (WSL4MIS,
DMSPS, CycleMix, ScribFormer); WORD stays a full-3D VNet pipeline, see
``train_pce_3d.py``. Only sparse labels in ``labelsTr`` contribute to
optimization. Dense training labels are accessed exclusively for model
selection on a patient-level holdout. The official ``imagesTs``/``labelsTs``
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D U-Net from ScribbleBench scribbles with pCE"
    )
...
def validate_args(args):
    ...
    # UNet2D downsamples H/W by 16 (4 maxpools) with its fixed architecture.
...
def resolve_case_split(cases, dataset_name):
    """Case-level (patient) train/val split, reusing common_3d's published-
    group resolver against this dataset's per-slice ``cases`` list."""
    probe_samples = [{"case": case} for case in cases]
    return make_published_split(probe_samples, dataset_name)


def build_val_dataset(args):
    # Local import: avoid paying for a second full-volume load path unless
    # validation actually needs it (per-case, full slice-stack access).
    from dataloader.scribblebench_3d import ScribbleBench3DDataset

    return ScribbleBench3DDataset(
...
def train(args):
    ...
    output_dir = Path(
        args.output_dir or REPO_ROOT / "checkpoints" / "ScribbleBench_pCE" / args.dataset
    ).resolve()
