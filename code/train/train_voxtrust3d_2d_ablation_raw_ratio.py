"""Table 2 "w/o Wilson bound" design-choice ablation
...
``utils.voxtrust3d_ablation_step.voxtrust_step_knockout``: the Wilson lower
confidence bound in ``RollingCalibrationBuffer.fit_thresholds`` (Eq. 15) is
replaced by the raw sample ratio ``k/n`` (``calibration_estimator="raw"``).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train VoxTrust-3D/DCC on ScribbleBench ACDC/MSCMR, Wilson bound replaced by the raw ratio k/n"
    )
