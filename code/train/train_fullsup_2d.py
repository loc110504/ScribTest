"""UNet2D fully-supervised (dense-mask) upper-bound baseline for ACDC/MSCMR's
2D slice protocol.
...
are otherwise handled exactly as in ``train_pce_2d.py``/``test_pce_2d.py``:
the checkpoint is a plain ``UNet2D`` (``model_name="unet_2d"``), so
``test_pce_2d.py`` evaluates it directly.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D U-Net fully-supervised (dense masks) on ScribbleBench ACDC/MSCMR"
    )
...
def train(args):
    ...
    output_dir = Path(
        args.output_dir or REPO_ROOT / "checkpoints" / "ScribbleBench_FullSup" / args.dataset
    ).resolve()
    ...
    train_dataset = ScribbleBench2DDataset(
        args.dataset, base_dir=args.root_path, split="train", sup_type="dense", transform=train_transform
    )
    ...
    logging.info(
        "dataset=%s classes=%d train_slices=%d val_cases=%d patch=%s device=%s (fully-supervised, dense masks)",
        args.dataset, num_classes, len(train_slice_positions), len(val_case_indices), args.patch_size, device,
    )
