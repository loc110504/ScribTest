"""Table 2 "w/o learned signal" design-choice ablation
...
``utils.voxtrust3d_ablation_step.voxtrust_step_knockout``: the reliability
score ``R_i`` (Eq. 5-7, margin x student-teacher agreement) is replaced
everywhere downstream (calibration-buffer updates and the acceptance test
alike) by the teacher's plain top-1 softmax probability
(``utils.voxtrust3d.select_reliability``, ``reliability_signal="top1_confidence"``).
Held-out calibration and distance conditioning are otherwise unchanged, and
Trust-Advantage EMA (this project's own extension, not part of what Table 2
ablates) is not exposed here -- the EMA teacher update always uses the
fixed ``--ema_decay`` rate.
...
ABLATION_NAME = "top1conf"
CALIBRATION_ESTIMATOR = "wilson"
ABSTAIN_POLICY = "abstain"
RELIABILITY_SIGNAL = "top1_confidence"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train VoxTrust-3D/DCC on ScribbleBench ACDC/MSCMR, R_i replaced by top-1 teacher confidence"
    )
