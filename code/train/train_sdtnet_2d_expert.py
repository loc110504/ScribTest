"""UNet2D + SDT-Net (Nguyen et al. 2026) on the expert-scribble ACDC/MSCMR
archive (see ``dataloader/expert_scribble_2d.py``), *in addition to* (not
instead of) ``train_sdtnet_2d.py``'s ScribbleBench pipeline.

Reuses ``sdtnet_step`` from ``train_sdtnet_3d.py`` unchanged, exactly like
``train_sdtnet_2d.py`` does -- only the dataset source differs (see
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D U-Net from the expert-scribble ACDC/MSCMR archive with SDT-Net"
    )
...
def train(args):
    ...
    output_dir = Path(
        args.output_dir or REPO_ROOT / "checkpoints" / "ExpertScribble_SDTNet" / args.dataset
    ).resolve()
