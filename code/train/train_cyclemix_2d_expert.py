"""UNet2D + CycleMix (Zhang & Zhuang, CVPR 2022) on the expert-scribble
ACDC/MSCMR archive (see ``dataloader/expert_scribble_2d.py``), *in addition
to* (not instead of) ``train_cyclemix_2d.py``'s ScribbleBench pipeline.

Reuses ``cyclemix_step`` from ``train_cyclemix_3d.py`` unchanged, exactly
like ``train_cyclemix_2d.py`` does -- only the dataset source differs (see
``train_pce_2d_expert.py``'s module docstring for the full rationale and
``resolve_case_split``/``resolve_val_indices``/``build_val_dataset``, which
this script imports from there, mirroring how ``train_cyclemix_2d.py``
imports its own such helpers from ``train_pce_2d.py``).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2D U-Net from the expert-scribble ACDC/MSCMR archive with CycleMix"
    )
