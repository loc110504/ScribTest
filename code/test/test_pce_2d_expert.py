...
Mirrors ``test_pce_2d.py`` exactly, except the test volumes come from
``dataloader.expert_scribble_2d.ExpertScribbleVolumeDataset`` (this archive's
own ``ACDC_training_volumes``/``MSCMR_testing_volumes``, already dense-
labeled, no NIfTI needed) instead of ScribbleBench's ``imagesTs``/``labelsTs``.
See that module's docstring for why HD95/ASSD reported here are in pixel
units, not calibrated millimeters (no voxel spacing is stored in this
archive) -- do not directly compare these HD95/ASSD numbers against
``test_pce_2d.py``'s (Dice is unaffected and *is* directly comparable).
``--save_predictions`` is not supported here: there is no NIfTI affine to
attach a prediction volume to in this archive.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a UNet2D checkpoint on the expert-scribble ACDC/MSCMR archive"
    )
...
def evaluate(args):
    ...
    if dataset_name not in ("ACDC", "MSCMR"):
        raise ValueError("test_pce_2d_expert.py only evaluates ACDC/MSCMR checkpoints")
    if data_config.get("data_source") != "expert_scribble":
        raise ValueError(
            "checkpoint was not trained by a *_2d_expert.py script (data_config.data_source != "
            "'expert_scribble'); use test_pce_2d.py for a ScribbleBench checkpoint"
        )
    ...
    output_dir = Path(args.output_dir or REPO_ROOT / "results" / "ExpertScribble_pCE" / dataset_name).resolve()
