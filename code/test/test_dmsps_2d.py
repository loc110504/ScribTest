"""Test UNetCCT2D+DMSPS checkpoints on the official ScribbleBench ACDC/MSCMR
test split.

Mirrors ``test_pce_2d.py``; only the model class and its main-decoder-only
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a UNetCCT2D+DMSPS checkpoint")
...
def evaluate(args):
    ...
    if checkpoint.get("model_name") != "unet_cct_2d":
        raise ValueError("checkpoint was not produced by train_dmsps_2d.py")
    model_config = checkpoint["model_config"]
    data_config = checkpoint["data_config"]
    dataset_name = data_config["dataset"]
    if dataset_name not in ("ACDC", "MSCMR"):
        raise ValueError("test_dmsps_2d.py only evaluates ACDC/MSCMR checkpoints; use test_dmsps_3d.py for WORD")
    ...
    output_dir = Path(args.output_dir or REPO_ROOT / "results" / "ScribbleBench_DMSPS" / dataset_name).resolve()
